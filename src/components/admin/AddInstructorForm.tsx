  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded border border-dashed border-ink/20 p-4">
      <p className="text-xs text-ink-soft">
        {lockSlug
          ? "A editar formador existente — o slug não pode ser alterado aqui."
          : "Adicionar formador (ou reenviar o mesmo slug para editar um já existente)."}
      </p>
      ...
      <div className="flex items-center gap-3">
        <button type="submit" disabled={loading} className="btn-secondary">
          {loading ? "A guardar..." : "Guardar formador"}
        </button>
        {lockSlug && (
          <a href="/admin/formadores" className="text-xs text-ink-soft underline">
            Cancelar
          </a>
        )}
      </div>
