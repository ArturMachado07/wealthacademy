...
// Mesmo comportamento do sino do aluno/admin, a apontar para os endpoints
// da empresa — avisa quando um colaborador entra numa turma, a turma fica
// completa, ou é activada depois de paga.
export default function CompanyNotificationBell() {
  ...
  useEffect(() => {
    fetch("/api/empresas/notificacoes")
      .then((res) => res.json())
      .then((data) => {
  ...
  async function handleOpen() {
    ...
    if (next && unreadCount > 0) {
      ...
      fetch("/api/empresas/notificacoes/marcar-lidas", { method: "POST" }).catch(() => null);
    }
  }
        ...
        <div className="absolute right-0 top-12 z-30 w-80 max-w-[calc(100vw-2rem)] overflow-hidden rounded border border-ink/10 bg-white shadow-lg">
          ...
          <div className="max-h-80 overflow-y-auto">
            {!loaded ? (
              ...
              <p className="px-4 py-6 text-center text-xs text-ink-soft">Ainda não há notificações.</p>
            ) : (
              notifications.map((n) => {
                const content = (
